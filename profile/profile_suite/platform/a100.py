"""A100 CUDA platform adapter.

:class:`A100CudaPlatform` is the concrete :class:`~profile_suite.platform.base.Platform`
implementation for an NVIDIA A100 GPU running the CUDA build of llama.cpp
(R6.3). It answers the four Platform questions for a campaign:

- ``descriptor`` is the stable string ``"a100-cuda"`` used to tag every result
  record and the Run_Manifest (R6.5).
- ``resolve_binary(kind)`` maps the binary roles ``"server"`` / ``"bench"`` /
  ``"batched-bench"`` onto the configured CUDA binary paths (R6.3).
- ``device_total_mib(gpu_index)`` reports the GPU's total device memory using the
  shared :class:`~profile_suite.harness.sysprobe.SysProbe` probe.
...
Feasibility heuristic (documented):
    ...
    explicit ``memory_budget_mib`` passed by the campaign, or the GPU's queried
    total device memory -- this adapter applies a conservative lower-bound
    estimate of the KV-cache footprint (which grows with ``ctx_length`` and
    ``slot_count``) and declines a Config only when even that lower bound already
    exceeds the budget. Because the estimate excludes model weights and compute
    scratch, it never *over*-accepts: a Config it rejects is genuinely infeasible.
    The estimate is intentionally simple; the Memory_Profiler measures the true
    peak and records exact shortfalls at run time (R3.5).
"""
...
#: Stable Platform descriptor for the A100 CUDA target (R6.5).
A100_DESCRIPTOR = "a100-cuda"
...
#: ever declined when a Config is clearly infeasible.
DEFAULT_KV_MIB_PER_KTOKEN_PER_SLOT = 8.0


class A100CudaPlatform:
    """Platform adapter for an A100 GPU running the CUDA llama.cpp build.

    Constructed with the campaign's configured CUDA binary paths and the pinned
    GPU index. Conforms structurally to the
    :class:`~profile_suite.platform.base.Platform` Protocol.
    """

    descriptor: str = A100_DESCRIPTOR
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            server_binary: Absolute path to the CUDA ``llama-server`` binary.
            bench_binary: Absolute path to the CUDA ``llama-bench`` binary.
            batched_bench_binary: Absolute path to the CUDA
    ...
    # ------------------------------------------------------------------ #
    # Binary resolution (R6.3)
    # ------------------------------------------------------------------ #
    def resolve_binary(self, kind: BinaryKind) -> str:
        """Return the configured CUDA binary path for ``kind``.
    ...
    def _effective_budget(self) -> float | None:
        ...
        to the GPU's queried total device memory. A failed/unavailable query
        yields ``nan``, which the caller treats as "no usable budget".
        """
